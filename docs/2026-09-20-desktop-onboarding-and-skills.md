...
| The startup page comes before the project, every launch, and picks one agent | The first thing on screen is which agent to work with, not a folder panel. There are two cards, Claude Code and Codex. Each says whether it was found and carries the evidence as its tooltip, and the first found one starts selected. A card with nothing found cannot be chosen: copying skills for an agent that is not there helps nobody, so it links to its maker's page instead, and with neither found Next is off. The other researched agents stay in the table, unoffered, until one is asked for. The page picks one agent, not a set, because a project is worked on with one agent at a time. A second agent is one more launch, which costs nothing because the install is idempotent. It is a page in the app's one window, at the canvas's size, and the canvas replaces it. Next leads to its second step, open-or-create, and Create to a third, the name, each with Back, and a numbered rail on the panel's left says which of the three is up. The second step also lists the folders in `Documents/Super Prototyping` in a dropdown under a search field, last edited first, each row with an icon, its name, where it is and when it was last edited. Typing narrows the rows by name, and a click, or Enter for the first row left, opens one with the agent from step one. The icon is the `icon.png` of the project's first canvas that has one, so a project looks like the app it prototypes, and a folder until it has one. The list is read from disk at launch and never stored, so the app still remembers nothing. Its cards carry a title and no helper text, so the projects are what the second step shows. `open -a … --args <dir>` skips the page, for a scripted launch, and installs nothing. |
...
the card's tooltip, and starts on the first card with something in it. A card
with nothing found is disabled and links to where to get the agent.
