...
steps: which agent, then open or create a project, then the new project's name. Nothing of the
app showed until all three were answered. Since then the agent's panel became the window's, one
...
| One question, the agent | It is the one thing the app cannot default. No email, no account: there is no backend to send it to yet. When there is, a field goes in this modal. |
| The chat panel's agents, not the app's own table | The modal lists `GET __sp/agent/agents`, which is what the panel can run, and whether each answers `--version` here. The app's own PATH, home directory and `.app` detection, the nineteen researched agents and their icons are gone. `launch.ts` keeps only the two rows' skills directories. |
...
| The skills go into the project behind it at once | `startup:agent` installs them into the project the window opened on, and every project opened after it gets them as before. The toast that names the copies is not shown for this one, because home has no canvas to show it. Since `2026-09-22-one-server-mode.md` the install is the server's `POST /p/<name>/__sp/skills`, which `startup:agent` sends until the page sends it itself. |
