...
2026-09-19. Issue #111 asked for a window around the canvas that `sp start`
serves, for the person who does not work in a terminal, and said to ship a dmg
only if people asked. The maintainer decided the dmg is worth doing now. This
note records which shell was picked and what the app has to keep in step with
the command line.
...
spawns agents, watches the boards directory and serves files. Tauri would have
to carry it as a sidecar with a Node runtime beside it, or write the endpoints
again in Rust, which is the duplication the rule forbids. Electron has Node
built in, so the server runs as a `utilityProcess` of the app. That is one
runtime, no sidecar, and one place the port comes from. The cost is a dmg of
about 130 MB per architecture instead of about 10. The people this is for will
not weigh that against opening a terminal. The example canvases the app has
shipped since then are most of what the dmg weighs now, and
...
- **The directories.** `SUPER_PROTOTYPING_HOME`, `XDG_STATE_HOME` and the
  ...
  rather than `~/Library/Application Support`. Nothing migrates and `sp clean`
  removes it. The app has no App Sandbox, because it reads boards from
  whichever project directory the user picks.
- **The port.** The order is `--port`, then `SP_CANVAS_PORT`, then 5173. The
  canvas document is stored in IndexedDB under the origin, so a stable port is
  what keeps a user's canvas from one launch to the next. The app takes a free
  port only when 5173 already answers, which means a canvas the command line
  started. A port that was asked for and is taken is an error before anything
  starts, not a second server racing the first. Electron loses a utility
  process's early output when a window is being created as it starts, so the
...
- **The plugin root.** The app does not ask for it. It is built with one
  ...
  dmg and the app no longer mentions it. `uv tool install` is its installer on
  every product, and the skills the app installs tell the agent to run it,
  pinned to the app's own version. A GUI app does not inherit the shell's
  PATH, so the app appends `~/.local/bin`, `~/.bun/bin`, `/opt/homebrew/bin`,
  `~/.cargo/bin` and `/usr/local/bin` to it first, and the server and every
  agent it spawns inherit the result.

## What was deliberately left out

- **Remembering anything.** The app keeps no last project, no window position
...
- **A universal binary, a cask, a version-skew dialog.** One runner builds two
  ...
  skew note still covers a terminal user who mixes an old toolkit with a newer
  plugin checkout.
...
and every identifier around them, are kept outside the repo.
