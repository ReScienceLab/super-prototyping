...
Two command-line tools, packaged so the skills can call them by name.
...
- **`refkit`** — the measuring toolkit. Overlays a labelled grid on a
  ...
  render against its source. `refkit --help` lists all eighteen subcommands.
...
Installing them puts `refkit` and `artgen` on `PATH`, and every skill reads
