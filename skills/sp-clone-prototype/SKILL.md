...
license: Apache-2.0
compatibility: Requires the refkit and artgen commands, which the Super Prototyping app puts on PATH (sp-prototype-canvas's scripts/install.sh or install.ps1), plus Google Chrome for the shoot subcommand. Reference captures are supplied by you; nothing is fetched.
metadata:
...
automatically. The `sp-prototype-canvas` skill covers how folders become tldraw
...
`sp`, `refkit` or `artgen` not found? Run `sh <sp-prototype-canvas skill dir>/scripts/install.sh` (Windows: `install.ps1`)
...
Hard constraints from the canvas renderer (also in `sp-prototype-canvas`'s
