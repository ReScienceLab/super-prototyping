...
open, and refuses a port that already answers rather than reusing it.

From elsewhere, name the project: `sp start <dir>`. Not on PATH, or `sp
--version` prints something lower than the version this skill shipped with
(never higher — that would be a downgrade, and the tag may not even exist)?
`sp` installs separately from the plugin, which cannot run an installer of
its own: `uv tool install
