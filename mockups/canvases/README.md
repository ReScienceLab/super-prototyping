...
- `substack-ios/`: the run to read when a translucent bar sits over content
  ...
  Mean absolute delta against those captures is 1.35-2.94 levels (of 255), mean
  2.08. Its `README.md` records the two probes that show iOS 26's Liquid Glass
  is not a linear alpha blend - the same tab oval darkens its material by 18
  ...
  one crop because Substack bakes its title into the image server-side, and a
  scrolled header band is a crop for the harder reason that no CSS filter can
  blur pixels the board does not have; and that a note's name is not a fixed
  distance below its avatar, so `note()` takes a `dy` that moves the two type
  rows and never the measured crop - eleven of those, each a third or two
  thirds of a point, were worth more than the blur fit and the compose glyph
  together. The seven `ref-*` boards are gitignored, so a fresh clone has 10.
