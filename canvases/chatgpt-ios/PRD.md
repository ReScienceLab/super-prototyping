...
## Users
Primary: a new visitor moving through sign-up — email or an OAuth-style provider, a verification code, an age check, a use-case question, a notifications prompt — on the way to a first message. Secondary: a returning signed-in user who asks questions from Home, attaches content from the composer, and finds earlier conversations and projects through the sidebar.
